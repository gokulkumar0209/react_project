...
let reel_JSON
let reel_data
let reel_to_JSON = JSON.stringify(reels);
if (!localStorage.getItem("reels")) {
	localStorage.setItem("reels", reel_to_JSON);
}
reel_JSON = localStorage.getItem("reels");
reel_data = JSON.parse(reel_JSON);

function App() {
	const [reelsX, setReels] = useRef(reel_data);
	return (
		<>
			<div>
				...
				<div className="container">
					<Routes>
						<Route path="/" element={<AllMovies reels={reelsX} />}></Route>
						<Route
							path="/new-movies"
							element={<NewMovies setReels={setReels} />}
						></Route>
